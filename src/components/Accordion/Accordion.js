...
export default function Accordion({ title, desc, index }) {
  return (
    <div className="relative mb-2 border dark:border-gray-700 border-gray-200">
      ...
      <label className={`${styles.title} block bg-white dark:bg-slate-700 p-3 md:p-4 font-semibold cursor-pointer text-[#C21B78] dark:text-gray-50 text-base md:text-xl`} htmlFor={`toggle${index}`}>
        { title }
      </label>
